...
public class sanPablo {
	...
	public Libro devolverLibro(String aBuscar) {
		...
		try {
			...
			//Creo el libro
			libro = new Libro (titulo.text(), isbn.text(), autores, editorial.text(), fecha.text(), paginas.text(), ofertaBarata.getUrl(), imagen.attr("src"), ofertaBarata);
...
/***********************************METODOS INTERNOS A LA CLASE********************************************************/	
	...
	private Oferta comparar(ArrayList<Oferta> ofertas) {
		...
		for (Oferta o: ofertas) {
			if(!o.getDisponibilidad().contains("Sin stock") && !o.getDisponibilidad().contains("No disponible")) {
